...
df = - 0.5D0*( &
     & (dphi_i*fj(2)*fj(1) + phi_i*dfjdr(2)*fj(1) - &
     &  phi_i*fj(2)*dfjdr(1))/fj(1)**2 + &
     & (dphi_j*fj(1)*fj(2) + phi_j*dfjdr(1)*fj(2) - &
     &  phi_j*fj(1)*dfjdr(2))/fj(2)**2 )/r
!
RETURN
...
fe = oxygn%f_e; gamm = oxygn%gamm; nu = oxygn%nu
